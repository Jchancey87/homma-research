...
* **Sparkline ([Sparkline.tsx](file:///home/jackc/projects/homma-research/frontend/components/Sparkline.tsx))**: Added optional `color` property and `colorByLast5m` property. If `colorByLast5m` is enabled, colors the sparkline green if the last 5-minute candle was green, and red if it was red. Wired this property into [GainerTable.tsx](file:///home/jackc/projects/homma-research/frontend/components/live-gainers/GainerTable.tsx) for row and detail view sparklines.
* **Dashboard Page ([page.tsx](file:///home/jackc/projects/homma-research/frontend/app/page.tsx))**: Removed the blocking server-side data fetch (`getDashboardOverview`) from the root `DashboardPage` server component. This allows the layout shell and table skeletons to stream and paint instantly (lowering TTFB/LCP). Component data will load asynchronously in parallel on client mount.
* **Layout Shift Optimization (CLS)**: Optimized headers, table containers, status metrics, and loading skeletons to eliminate visual layout shifts (CLS):
  * **Dashboard Header ([DashboardHeader.tsx](file:///home/jackc/projects/homma-research/frontend/components/DashboardHeader.tsx))**: Initialized state values (`timeDetails`, `currentTimeET`) during state declaration rather than on mount, preventing empty date flashes. Set a stable `lg:w-[480px] lg:justify-center` desktop width on the center session strip to anchor sibling action buttons.
  * **Table Containers ([GainerTable.tsx](file:///home/jackc/projects/homma-research/frontend/components/live-gainers/GainerTable.tsx))**: Replaced dynamic table height with a stable scroll container (`h-[320px] overflow-y-auto`) to lock layouts in place when gainers resolve or update.
