...
* **mom_2m Calculation:** `best = min(candles, key=lambda c: abs((c.get('t') or 0) - target_ts))` where `target_ts = now_ms - 120_000`. Use `best['c']` as base price. Single line.
* **30s Cache:** `_compute_minute_metrics` caches for 30s; updates metrics inline using cached `price_2min_ago` and `atr_14`.
